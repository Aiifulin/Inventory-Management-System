...
async function logActivity(action, targetName) {
    try {
        ...
        await addDoc(collection({}, "activities"), {
            action,
            target: targetName,
            user: userEmail,
...
async function addCategory(categoryData) {
    const snapshot = await getDocs(collection({}, "categories"));
    ...
    await setDoc(doc({}, "counters", "categories"), { lastId: newIdNum });

    await setDoc(doc({}, "categories", newId), {
        ...categoryData,
